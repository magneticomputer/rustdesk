...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/MagneticSupport.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"Magnetic Support Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon MagneticSupport.app 200 190 --hide-extension MagneticSupport.app rustdesk.dmg ./build/macos/Build/Products/Release/MagneticSupport.app")
...
def main():
    ...
    if windows:
        ...
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/MagneticSupport.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/MagneticSupport.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/MagneticSupport.app/Contents/MacOS/rustdesk
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/MagneticSupport.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/MagneticSupport.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/MagneticSupport.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/MagneticSupport.app
    '''.format(pa))
                system2(
                    'create-dmg "MagneticSupport %s.dmg" "target/release/bundle/osx/MagneticSupport.app"' % version)
    ...
    # verify:  spctl -a -t exec -v /Applications/MagneticSupport.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
